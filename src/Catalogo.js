...
const Catalogo = ({ agregarProductoACatalogo, catalogoPersonal }) => {
  ...
  const proveedores = [
    ...new Set(productos.map((producto) => producto.Proveedor)),
  ];
  ...
  const filtrarProductos = useCallback(() => {
    ...
    if (codigoProducto !== "") {
      productosFiltrados = productosFiltrados.filter((producto) =>
        producto.Codigo.includes(codigoProducto)
      );
    }
    if (descripcion !== "") {
      productosFiltrados = productosFiltrados.filter((producto) =>
        producto.Articulo_descripcion.toLowerCase().includes(
    ...
    if (rubroSeleccionado !== "") {
      productosFiltrados = productosFiltrados.filter(
        (producto) => producto.Rubro === rubroSeleccionado
      );
    }
  ...
  return (
    <div className="catalogo">
      ...
      <ul className="grid">
        {currentProducts.map((producto) => (
          <li key={producto.Codigo} className="producto">
            ...
            <button
              onClick={(e) =>
                descargarImagen(producto.Imagenes, producto.Codigo, e)
              }
              className="producto-boton"
              type="button"
            >
      ...
      {/* Paginación */}
      <div className="paginacion">{renderPageNumbers()}</div>
    </div>
  );
};
